...
| `kouba.ts` | kouba（工数管理）の認証（user_idスコープ）・テーブル用意（列追加のALTERも含む）・カテゴリ→タスク→サブタスクのまとめ取得（`loadBoard`＝カテゴリを取り、`kouba_task_categories`をIN句でまとめて引いて「タスクがどのカテゴリに属するか」のリンク一覧を得たあと、リンクのtask_idぶんだけ`kouba_tasks`/`kouba_subtasks`を引いてMapでぶら下げる＝**1タスクが複数カテゴリに属せる**ので、同じタスクの内容（同じサブタスク・同じ合計時間）を`shapeTask`で複数回組み立て、リンクのあるカテゴリの数だけ`tasks`配列に入れる〈同期はオブジェクトの使い回しではなく「同じDB行から毎回作る」ことで保証している〉。リンクの並び順は`sort_order ASC`、サブタスクは`created_at ASC`）・タスク/カテゴリの合計時間計算（`shapeTask`/`shapeCategory`。専用の集計列は持たずサブタスクの`hours`をその場でSUMし、それを1段ずつ積み上げる。サブタスク自体は`shapeSubtask`で行を整形するだけで集計はしない＝`hours`がそのまま値）・所有者チェック（`findOwnedCategory`/`findOwnedTask`〈返り値は`id`のみ＝`category_id`はもう意味を持たないので返さない〉/`findOwnedSubtask`/`ownedCategoryIds`〈複数カテゴリIDのうち本人所有ぶんの集合を返す。タスクのcategoryIds検証に使う〉）・タスクの所属カテゴリ一覧取得（`loadTaskCategoryIds`＝PATCH時の差分〈増えた分/外れた分〉計算に使う）・アイコンの正規化（`normalizeIcon`＝空ならフォールバック絵文字）・作業時間の正規化（`normalizeHours`＝30分刻みに丸めたうえで0〜30の外はnull。0は有効な値なのでfalsy判定で弾かないこと。`KoubaHoursStepper.vue` の +/- と表裏）・カテゴリ内タスクの末尾sort_order算出（`nextTaskSortOrder`＝`kouba_task_categories`をカテゴリで絞って最大値+1。新規作成時とカテゴリ追加〈モーダルのチップ〉の両方で使う。ドラッグ&ドロップでの並べ替えは`reorder.post.ts`が並び順を丸ごと受け取ってこの関数は使わない）・**達成したこと**のCRUD（`loadAchievements`/`createAchievement`/`findOwnedAchievement`/`deleteAchievement`。板・カテゴリとは無関係の単純なログで集計や積み上げは無い）・達成日の正規化（`normalizeAchievedAt`＝"YYYY-MM-DD"をJST正午基準でUTCのISO8601へ変換）・説明文の正規化（`normalizeDescription`＝trimのみ、`KOUBA_DESCRIPTION_MAX`〈500文字〉超過はnull。空文字＝説明なしは許す） |
