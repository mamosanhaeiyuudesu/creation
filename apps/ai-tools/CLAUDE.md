...
**2026-09-04にUIから外し、PDFダウンロードに置き換えた**（`index.vue`/`[id].vue` の連携ボタン・承認ボタンはコメントアウト。`kikigaki-google.ts` / `api/kikigaki/google/*` / `api/kikigaki/approve.post.ts` / `api/kikigaki/resend-sheet.post.ts` はファイルとして残しているが、どこからも呼ばれない）。再度有効化する可能性を考えて削除はしていないので、以下のセットアップ手順は再有効化するときのために残す。
...
| `kikigaki-google.ts` | kikigaki の Google OAuth2（PKCE、スコープ documents/spreadsheets/tasks/calendar）と **Docs/Sheets/Tasks/Calendar への書き込み全部**（`writeApprovedMinutes`）。**2026-09-04にUIから外し未使用**（`api/kikigaki/approve.post.ts` も同様。再有効化する可能性を考えて削除はしていない） |
