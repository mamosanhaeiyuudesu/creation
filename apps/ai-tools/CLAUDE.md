...
| `/kouba` | 工数管理。「マーケティング」「人間探求」のような**自分で決めたカテゴリ**を3×3の9枠のボードに並べ、その中に**タスクを付箋として置く**。各タスクを開くと日付・時間・箇条書きメモ（改行区切りで複数項目）を1件ずつ追記でき、**タスクの合計時間は付箋自体にバッジ表示、カテゴリの合計時間はカテゴリ枠のヘッダーに表示**（どちらもログをSUMして毎回出す値で、専用の集計列は持たない）。**カテゴリは9個で打ち止め**＝空き枠をクリックした位置（0〜8）がそのままDBの `position` になり、途中のカテゴリを消しても空いたその枠にしか次のカテゴリは入らない（`position` の空き番号をクライアントが選び、サーバーは範囲と重複だけ検証する）。**カテゴリ・タスクとも作成時に絵文字アイコンを選べる**（`KoubaIconPicker.vue`＝20種の候補＋自由入力欄。作成後もアイコン部分をクリックすればポップオーバーでいつでも変更できる。列は`icon`、既定値はカテゴリ📁／タスク📝）。**タスクは同一カテゴリ内でドラッグして並べ替え、別カテゴリの枠へドラッグして移動もできる**（付箋に`draggable`、カテゴリの付箋エリアとタスク自身の両方にdragover/drop。ドロップ先が別タスクの上なら「その直前に挿入」、カテゴリの空き領域なら「末尾に追加」。並び順は`kouba_tasks.sort_order`で永続化し、ドラッグのたび移動先カテゴリの全タスクIDを並び順どおりに`POST /api/kouba/tasks/reorder`へ送って一括更新する＝同一カテゴリの入れ替えも他カテゴリへの移動も同じ形。**タスク詳細モーダルのカテゴリ選択（`<select>`）からも移動できる**＝ドラッグと違って移動先の末尾に追加されるだけで並び順は指定しない、`PATCH /api/kouba/tasks/[id]`にcategoryIdを渡す形。**reorderのHTTPメソッドはPATCHではなくPOST**＝`tasks/[id].patch.ts`と同じ`/api/kouba/tasks/${id}`テンプレートパターンに`tasks/reorder.patch.ts`という同名メソッドの兄弟ルートを置くと、Nuxtの`$fetch`型推論が`[id]`動的ルートの許容メソッドを`reorder`側のPATCHだけに潰してしまい`deleteTask`のDELETE呼び出しが型エラーになる事故が実際に起きた＝動的ルートと同じ親配下に静的セグメントの兄弟ルートを足すときは、メソッドを重複させないこと。**カテゴリ・タスク・記録の削除はすべて`KoubaConfirmModal.vue`（自作のポップアップ）で確認**＝ブラウザ標準の`confirm()`は使わない（タスク削除・記録削除はタスク詳細モーダルの上に重ねて出るので`z-[1100]`で本体`z-[1000]`より前面に出す）。AIは使わない、手入力だけの集計ツール。既存 users/sessions 認証に相乗りし、カテゴリ・タスクは user_id でスコープ（`kouba_logs` だけ user_id を持たず `kouba_tasks` との join で所有者を確認する）。動作確認は `yarn dev` のローカルD1（miniflare。`.wrangler/state/v3/d1/`）に対して curl と Playwright で一通り実施済み（カテゴリ/タスク/ログのCRUD・アイコン変更・ドラッグでの並べ替えとカテゴリ移動・ドロップダウンでの移動・カスタム削除確認ポップアップ、いずれも想定どおり）。**ローカルdevでもD1は使える**（`nuxt dev` が「cloudflare-dev emulation」で自動的にminiflare D1を用意する。他ツールの一部の行にある「ローカルdevはD1が使えない」という記述は要再確認＝少なくとも現時点のこのリポジトリでは誤り） |
...
- `WHISPER_DB` 相乗り（kouba）: kouba_categories（カテゴリ。`position` 0〜8がそのまま3×3グリッドの位置、`icon`は絵文字アイコン）/ kouba_tasks（タスク＝付箋。`category_id`＋`user_id`＋`icon`＋`sort_order`〈カテゴリ内の並び順。ドラッグ&ドロップとカテゴリ間移動の両方で使う〉を持つ）/ kouba_logs（作業ログ＝日付・時間・箇条書きメモを1件ずつ。`user_id`は持たず`kouba_tasks`とのjoinで所有者を確認する）。タスク・カテゴリの合計時間は列を持たず、表示のたびに`kouba_logs`をSUMして出す。既存 users/sessions 認証に相乗りし `user_id` でスコープ。テーブルは `ensureKoubaTables()` で自動生成もされる（`icon`/`sort_order`は後から足した列なので `CREATE TABLE IF NOT EXISTS` とは別に `ALTER TABLE ... ADD COLUMN` も個別に流している＝ローカルdevで先に古い形のテーブルを作ってしまっていても列が足りる）。
...
| `kouba.ts` | kouba（工数管理）の認証（user_idスコープ）・テーブル用意（列追加のALTERも含む）・カテゴリ→タスク→ログのまとめ取得（`loadBoard`＝momoの`loadOrders`と同じ「親を取ってIN句で子をまとめて取り、Mapでぶら下げる」方式。タスクは`sort_order ASC, created_at ASC`で並べる）・タスク/カテゴリの合計時間計算（`shapeTask`/`shapeCategory`。専用の集計列は持たずログをその場でSUM）・所有者チェック（`findOwnedCategory`/`findOwnedTask`）・アイコンの正規化（`normalizeIcon`＝空ならフォールバック絵文字）・カテゴリ内タスクの末尾sort_order算出（`nextTaskSortOrder`＝新規作成時とカテゴリ間移動〈ドロップダウン経由〉の両方で使う。ドラッグ&ドロップでの並べ替えは`reorder.post.ts`が並び順を丸ごと受け取ってこの関数は使わない） |
