...
15フィードの取得＋要約＋5潮流ぶんの考察更新を1回の呼び出しに詰め込むと簡単に超え、実際に
...
**潮流分類と考察**: 記事は要約と同時に `src/utils/news-currents.ts` の5潮流（実世界に手を伸ばすAI／知的労働の自動化／
土台を巡る競争／安全保障とガバナンス／心と健康に触れるAI）のどれか1つにAIが分類する（`news-ai.ts` の `summarizeArticle`）。
ソースと潮流は固定対応ではなく記事ごとに判定する（IEEE Spectrumの記事でも内容次第で「知的労働の自動化」になり得る）。
**その日に新着があった潮流だけ**、直近30日分の見出し一覧（`NEWS_TREND_LOOKBACK_DAYS`）と前回の考察を材料に
`news-ai.ts` の `synthesizeCurrentNarrative` が「いまの考察」を書き直し `news_currents` に保存する
（履歴は持たず1潮流1行を上書き）。新着が無かった潮流は据え置き＝5潮流ぶん毎回呼ぶ無駄を避けている。
ページ上段の5枚のカードがこれで、クリックすると下の記事一覧がその潮流だけに絞り込まれる（大きな流れ→個別記事のドリルダウン）。
...
| `/news` | AIニュース朝刊。15フィード（OpenAI/DeepMindの企業ブログ＋ロボティクス・農業・マーケ・計算基盤・地政学・安保・医療の専門メディア）を3日に1回7時(JST)に集め、Claude（Haiku 4.5）が日本語3〜5行の要約・重要度1〜5・5潮流のどれかへの分類を付ける。**メールも通知も無い**（コストが見合わずやめた。経緯は上のセットアップ節）。**上段に5潮流のカード**＝直近1ヶ月を踏まえてAIが書いた「いまの考察」（カードは`line-clamp`で4行に切って途中まで）。クリックすると全文をポップアップで表示する（ここまでの流れ／今日の新着／これから、の3章立て・見出しつき）。ポップアップ内の「この潮流の記事だけ見る」で下の記事一覧をその潮流だけに絞り込んで一覧までスクロールする（大きな流れ→全文→個別記事のドリルダウン）。下段は日付ごとの記事一覧で、潮流／ソース／重要度／キーワードでも絞り込める。既定は重要度3以上の表示で、2以下も保存してあり切り替えれば読める。ヘッダの「いま収集する」で cron を待たずに手動実行（内部では収集→考察更新の2リクエストを順に呼ぶ。理由は上のセットアップ節）。下部の `<details>` に実行ログ（`news_runs`） |
| `/office` | 勤怠管理（日付・打刻記録） |
| `/games` | ゲーム一覧（リンク集） |
...
- `WHISPER_DB` 相乗り（news）: news_items（記事1件＝1行。`url` が UNIQUE ＝これが「処理済みURL一覧」の役割を果たすので、
  差分判定のための別ファイルは持たない。`importance` 1〜5 / `current` は5潮流のid（AIが記事ごとに分類）/
  `body_source` は article か feed / `digest_date` は収集した日(JST)）/ **news_currents**（潮流ごとの「いまの考察」。
  5潮流ぶん、id ごとに1行だけ持ち履歴は残さない。`narrative` は直近30日の一覧＋前回の考察を材料にAIが書き直す）/
...
| `news-ai.ts` | news の要約＋重要度判定＋5潮流への分類（`callClaudeText` を使う）。JSONで返させて `extractJson()` で取り出す。`synthesizeCurrentNarrative()` は潮流ごとの「いまの考察」を直近30日の一覧＋前回の考察から3章（`TREND_SECTION_TITLES`）で書き直す。`stripMarkdown()` / `breakSentences()` で出力を後処理（プロンプトの指示だけでは守られないため） |
