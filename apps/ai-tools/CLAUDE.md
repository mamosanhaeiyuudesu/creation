...
- `WHISPER_DB` 相乗り（task）: task_profiles（Trelloの接続情報。key/token は暗号化）/ **task_sns_posts**（投稿カウンター＝インスタ・note・Facebook の投稿数を「日 × プラットフォーム」で1行。0件の日は行を持たない＝累計は SUM で出す。プラットフォームを増やしてもスキーマ変更は不要）。既存 users/sessions 認証に相乗りし `user_id` でスコープ。テーブルは `ensureTaskSnsTables()`（`server/utils/task-sns.ts`）で自動生成もされる。**ローカルdevはD1が無いので localStorage に保存する**（プロフィール設定と同じ方式。`useTaskProfiles` / `useTaskSns` が `import.meta.dev` で分岐）。
...
| `guesthouse-insights.ts` | guesthouse **顧客分析の匙加減を集約**（滞在の型の判定基準・満足度の側面〈体験もここに統合。不便の受け皿として`その他`あり〉と関心の対象の**固定語彙**・感想の主語 `ASPECT_SUBJECTS`＝この宿/宿坊・ほかの宿/エリア・旅程に入った理由 `VISIT_REASONS`・宿の体験とツアーの体験の分け方・不便を遠慮なく拾わせる指示・旅程の順番の書かせ方・抽出システムプロンプト・**分析軸を提案させるプロンプト** `buildAxisSuggestSystem`・`VOCAB_VERSION`）。旅程の位置計算だけは表示側でも使うので `src/utils/guesthouse-route.ts`（純粋関数）に分けてある。AIに自由にタグを振らせると「食事/料理/ごはん」が別タグになり時系列で見ているものが語彙のブレになるため語彙を閉じている。**語彙や方針を変えたら `VOCAB_VERSION` を +1**（全プロファイルが作り直される）。分析軸の変更は基本ここだけ編集 |
