...
- `WHISPER_DB` 相乗り（keiko）: keiko_members（メンバー。ユーザーごとに管理、既定値で護/匡/真啓をシード）/ keiko_items（練習項目。**`member_id` でメンバーごと**に持つ。`rep_count`＝本数・`point_per_rep`＝1本あたりのポイント・`active`列で表示/非表示）/ keiko_records（花丸＝メンバー×項目×日で1件。存在する＝できた、`(member_id, item_id, date)` に一意制約。**ポイントは持たず**、集計時に `keiko_items` と JOIN して `rep_count * point_per_rep` を SUM する）。既存 users/sessions 認証に相乗りし、すべて `user_id` でスコープ。テーブルは `ensureKeikoTables()` で自動生成もされる（列追加も `ALTER TABLE ... ADD COLUMN` を握りつぶしつつ実行）。
  - 適用: `042_keiko.sql` → `044_keiko_points.sql`（項目のメンバー分け＋本数/ポイント列。ALTER 含むので再実行不可）を順に `wrangler d1 execute whisper-db --remote --file ...`
...
| `keiko.ts` | keiko の認証（user_idスコープ）・テーブル用意/列追加・メンバー/練習項目/花丸記録の読み書き・**ポイント集計**（`loadPointBuckets`＝メンバー×日 または メンバー×月で `rep_count * point_per_rep` を SUM）・初回アクセス時の既定メンバー（護/匡/真啓）と既定項目のシード・旧データ（メンバー共通項目）の移行 |
