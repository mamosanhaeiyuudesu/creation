...
| `/kouba` | 工数管理。「マーケティング」「人間探求」のような**自分で決めたカテゴリ**を3×3の9枠のボードに並べ、その中に**タスクを付箋として置く**。各タスクを開くと日付・時間・箇条書きメモ（改行区切りで複数項目）を1件ずつ追記でき、**タスクの合計時間は付箋自体にバッジ表示、カテゴリの合計時間はカテゴリ枠のヘッダーに表示**（どちらもログをSUMして毎回出す値で、専用の集計列は持たない）。**カテゴリは9個で打ち止め**＝空き枠をクリックした位置（0〜8）がそのままDBの `position` になり、途中のカテゴリを消しても空いたその枠にしか次のカテゴリは入らない（`position` の空き番号をクライアントが選び、サーバーは範囲と重複だけ検証する）。AIは使わない、手入力だけの集計ツール。既存 users/sessions 認証に相乗りし、カテゴリ・タスクは user_id でスコープ（`kouba_logs` だけ user_id を持たず `kouba_tasks` との join で所有者を確認する）。動作確認は `yarn dev` のローカルD1（miniflare。`.wrangler/state/v3/d1/`）に対して curl と Playwright で一通り実施済み（カテゴリ/タスク/ログのCRUD・位置の重複と範囲チェック・カスケード削除・合計時間の再計算・付箋UIとモーダルの表示、いずれも想定どおり）。**ローカルdevでもD1は使える**（`nuxt dev` が「cloudflare-dev emulation」で自動的にminiflare D1を用意する。他ツールの一部の行にある「ローカルdevはD1が使えない」という記述は要再確認＝少なくとも現時点のこのリポジトリでは誤り） |
...
- `WHISPER_DB` 相乗り（kouba）: kouba_categories（カテゴリ。`position` 0〜8がそのまま3×3グリッドの位置）/ kouba_tasks（タスク＝付箋。`category_id`＋`user_id`を持つ）/ kouba_logs（作業ログ＝日付・時間・箇条書きメモを1件ずつ。`user_id`は持たず`kouba_tasks`とのjoinで所有者を確認する）。タスク・カテゴリの合計時間は列を持たず、表示のたびに`kouba_logs`をSUMして出す。既存 users/sessions 認証に相乗りし `user_id` でスコープ。テーブルは `ensureKoubaTables()` で自動生成もされる。
...
| `kouba.ts` | kouba（工数管理）の認証（user_idスコープ）・テーブル用意・カテゴリ→タスク→ログのまとめ取得（`loadBoard`＝momoの`loadOrders`と同じ「親を取ってIN句で子をまとめて取り、Mapでぶら下げる」方式）・タスク/カテゴリの合計時間計算（`shapeTask`/`shapeCategory`。専用の集計列は持たずログをその場でSUM）・所有者チェック（`findOwnedCategory`/`findOwnedTask`） |
