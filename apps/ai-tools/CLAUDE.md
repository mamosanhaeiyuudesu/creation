...
| `kouba.ts` | kouba（工数管理）の認証（user_idスコープ）・テーブル用意（列追加のALTERも含む）・カテゴリ→タスク→サブタスクのまとめ取得（`loadBoard`＝momoの`loadOrders`と同じ「親を取ってIN句で子をまとめて取り、Mapでぶら下げる」方式を1段深く繰り返す。タスクは`sort_order ASC, created_at ASC`、サブタスクは`created_at ASC`で並べる）・タスク/カテゴリの合計時間計算（`shapeTask`/`shapeCategory`。専用の集計列は持たずサブタスクの`hours`をその場でSUMし、それを1段ずつ積み上げる。サブタスク自体は`shapeSubtask`で行を整形するだけで集計はしない＝`hours`がそのまま値）・所有者チェック（`findOwnedCategory`/`findOwnedTask`/`findOwnedSubtask`）・アイコンの正規化（`normalizeIcon`＝空ならフォールバック絵文字）・作業時間の正規化（`normalizeHours`＝30分刻みに丸めたうえで0〜30の外はnull。0は有効な値なのでfalsy判定で弾かないこと。`KoubaHoursStepper.vue` の +/- と表裏）・カテゴリ内タスクの末尾sort_order算出（`nextTaskSortOrder`＝新規作成時とカテゴリ間移動〈ドロップダウン経由〉の両方で使う。ドラッグ&ドロップでの並べ替えは`reorder.post.ts`が並び順を丸ごと受け取ってこの関数は使わない） |
