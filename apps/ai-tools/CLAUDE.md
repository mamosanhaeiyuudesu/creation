...
| `/news` | AIニュース朝刊。15フィード（OpenAI/DeepMindの企業ブログ＋ロボティクス・農業・マーケ・計算基盤・地政学・安保・医療の専門メディア）を毎朝7時(JST)に集め、Claude（Haiku 4.5）が日本語3〜5行の要約・重要度1〜5・5潮流のどれかへの分類を付ける。**メールも通知も無い**（コストが見合わずやめた。経緯は上のセットアップ節）。**上段に5潮流のカード**＝直近1ヶ月を踏まえてAIが書いた「いまの考察」。クリックすると下の記事一覧がその潮流だけに絞り込まれる（大きな流れ→個別記事のドリルダウン）。下段は日付ごとの記事一覧で、潮流／ソース／重要度／キーワードでも絞り込める。既定は重要度3以上の表示で、2以下も保存してあり切り替えれば読める。ヘッダの「いま収集する」で cron を待たずに手動実行（内部では収集→考察更新の2リクエストを順に呼ぶ。理由は上のセットアップ節）。下部の `<details>` に実行ログ（`news_runs`） |
| `/office` | 勤怠管理（日付・打刻記録） |
| `/games` | ゲーム一覧（リンク集） |
