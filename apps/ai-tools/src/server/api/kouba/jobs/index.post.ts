import { requireKoubaUser, requireKoubaDb, ensureKoubaTables, ownedCategoryIds, normalizeIcon, nextJobSortOrder } from '~/server/utils/kouba'
import { KOUBA_DEFAULT_JOB_ICON } from '~/types/kouba'

// ジョブ（付箋）を新規作成する。1つ以上のカテゴリに同時掲載できる（categoryIds）＝
// 作成時は「＋」を押したその1カテゴリだけを渡す運用で、複数掲載は後からジョブ詳細モーダルで設定する。
// 各カテゴリ内では、そのカテゴリの末尾（sort_orderの最大+1）に追加する。
// 説明文は作成フォームには無く、作成後にジョブ詳細モーダルから編集する運用（description は任意）。
export default defineEventHandler(async (event) => {
  ...
  const icon = normalizeIcon(body?.icon, KOUBA_DEFAULT_JOB_ICON)
