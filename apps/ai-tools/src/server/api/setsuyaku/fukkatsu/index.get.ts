...
export default defineEventHandler(async (event) => {
  ...
  const rows = await db
    .prepare('SELECT id, date, name, price, reason, tags FROM setsuyaku_fukkatsu WHERE user_id = ? ORDER BY date DESC, created_at DESC')
    .bind(user.id)
    .all<{ id: string; date: string; name: string; price: number; reason: string; tags: string }>()
