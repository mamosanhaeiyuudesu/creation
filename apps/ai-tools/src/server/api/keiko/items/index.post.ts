...
export default defineEventHandler(async (event): Promise<KeikoItem> => {
  ...
  const body = await readBody<{ memberId?: string; name?: string; repCount?: number; pointPerRep?: number }>(event)
  ...
  await db
    ...
    .bind(id, user.id, memberId, name, repCount, pointPerRep, sortOrder)
    .run()

  return shapeKeikoItem({ id, member_id: memberId, name, rep_count: repCount, point_per_rep: pointPerRep, sort_order: sortOrder, active: 1 })
})
