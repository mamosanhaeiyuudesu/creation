...
// 傾向を再計算する（管理トップの「更新」ボタン）。
// 日記が前回計算時から変わっていなければ再計算せず、前回結果をそのまま返す（updated=false）。
export default defineEventHandler(async (event): Promise<TrendsRefreshResult> => {
  ...
  const { fingerprint, count } = await loadDiaryFingerprint(db, user.id)
  const stored = await loadStoredTrends(db, user.id)

  // 変化なし → 再計算しない。
  if (stored && stored.fingerprint === fingerprint) {
    return { items: stored.items, basedOn: stored.basedOn, computedAt: stored.computedAt, stale: false, updated: false }
  }

  // 日記が2件未満なら傾向は出さない（指紋だけ保存して次回スキップできるようにする）。
  if (count < 2) {
    await saveTrendsCache(db, user.id, [], fingerprint, count)
    const s = await loadStoredTrends(db, user.id)
    return { items: [], basedOn: count, computedAt: s?.computedAt ?? null, stale: false, updated: true }
  }
  ...
  return { items, basedOn: diaries.length, computedAt: s?.computedAt ?? null, stale: false, updated: true }
})
