...
interface GratitudeItem { title: string; weight: number; content: string }
interface GratitudeEntry { items: GratitudeItem[]; summary: string; generatedAt: string }
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        system: `あなたは日々の記録からユーザーが感謝している物事を洗い出すカウンセラーです。
提供されたデータ（日々の気持ち・状況の記録と頻出単語）をもとに、ユーザーが感謝している・ありがたく感じている対象を日本語で抽出してください。
記録は時期ごとに分けて与えられます（各見出しは「2025年5〜8月頃」のように実際の年月）。新しい時期の記録だけに偏らず、すべての時期に目を通したうえで、期間全体から感謝を拾い上げてください。期間に言及するときは「初期」「中期」などの抽象語ではなく、与えられた見出しの実際の年月で具体的に書いてください。

必ず以下のJSON形式のみで返答してください（マークダウンコードブロックや説明文は一切不要）:
{"items":[{"title":"感謝のタイトル（8文字以内でできるだけシンプルに）","weight":8,"content":"説明（150字以内）"}],"summary":"感謝全体を俯瞰して伝える全体コメント（120字以内）"}

ルール:
- items は 20 項目程度（最低でも 15 項目）抽出する
- 感謝の対象は人・支え・出来事・環境・自分自身への感謝など幅広く拾う。明示的に「ありがとう」と書かれていなくても、記録から読み取れる支えや恵まれている点を含めてよい
- 似た内容の重複や、些細な一コマの言い換えを並べず、ユーザーの記録全体を漏れなくカバーしながら（MECE: 互いに排反かつ全体を網羅）、本人にとって大きな支えになっているものを優先して選ぶ
- weight は 1〜10 の整数で、その感謝の大きさ・重要度を表す（大きいほど treemap で大きく表示される）
- title は必ず 8 文字以内でできるだけシンプルにする（treemap 上に表示されるため）。content は何に対してどう感謝しているかを具体的に記述する
- summary はユーザーが恵まれている点を温かく俯瞰して伝えるコメントにする`,
    ...
    let parsed: { items: GratitudeItem[]; summary: string }
    try {
    ...
    const clampItem = (l: GratitudeItem): GratitudeItem => ({
    ...
    if (db && user) {
      const existing = await db
        .prepare('SELECT data FROM hagemashi_gratitude WHERE user_id = ?')
        .bind(user.id)
      ...
      await db
        .prepare("INSERT OR REPLACE INTO hagemashi_gratitude (user_id, data, updated_at) VALUES (?, ?, datetime('now'))")
    ...
    return wrapApiError(err, '感謝マップの生成に失敗しました')
  }
})
