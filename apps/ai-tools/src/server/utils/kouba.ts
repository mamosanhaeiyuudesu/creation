...
//
// **命名とテーブルの対応**（2026-09-15、呼び名を1段ずつ繰り下げた際の対応表。詳しい経緯は
...
// **UI「サブタスク」= 型 KoubaSubtask = DBテーブル kouba_task_subtasks は上記の階層と無関係な独立機能**
// （名前だけのTODOリスト。2026-09-17に作り直した。詳しくは types/kouba.ts 冒頭コメント参照）。
...
export async function ensureKoubaTables(db: any): Promise<void> {
  const statements = [
    ...
    // UI「サブタスク」（2026-09-17作り直し）。板（カテゴリ→ジョブ→タスク）とは無関係な、名前だけの独立したTODOリスト。
 ...
 * （板とは無関係の独立した機能のため、`loadSubtaskList()` で別に取得する）。
...
export async function findOwnedSubtaskItem(db: any, userId: string, subtaskId: string): Promise<{ id: string } | null> {
  const row = await db
    .prepare('SELECT id FROM kouba_task_subtasks WHERE id = ? AND user_id = ?')
...
/** ユーザーが次に使う sort_order（末尾に追加するサブタスクの値）。 */
export async function nextSubtaskSortOrder(db: any, userId: string): Promise<number> {
  const row = await db
    .prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM kouba_task_subtasks WHERE user_id = ?')
    .bind(userId)
    .first<{ m: number }>()
  return (row?.m ?? -1) + 1
}
