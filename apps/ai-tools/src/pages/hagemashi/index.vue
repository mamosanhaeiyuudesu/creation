<template>
  <div class="flex flex-col items-center px-4 pt-4 lg:pt-8 pb-4 h-full" @click="showSettingsMenu = false">
    ...
    <div class="relative z-50 w-full max-w-[600px] ml-2.5 flex-1 min-h-0 flex flex-col">
      ...
      <div class="w-full bg-white/[0.04] border border-white/[0.08] rounded-2xl p-7 shadow-[0_20px_80px_rgba(0,0,0,0.35),0_0_40px_rgba(249,115,22,0.06)] backdrop-blur-[10px] flex flex-col gap-4 flex-1 min-h-0 overflow-hidden">
      ...
      <div class="mt-1 min-w-0 flex-1 min-h-0 flex flex-col">
        ...
        <!-- タブ内容（残りの縦幅いっぱいに広げ、内部スクロールさせる） -->
        <div class="flex-1 min-h-0 overflow-y-auto [scrollbar-width:thin] [scrollbar-color:rgba(249,115,22,0.3)_transparent]">
        <HistoryTable
