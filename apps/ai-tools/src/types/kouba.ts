// 工数管理ツール (kouba) の型定義。
//
// **階層は カテゴリ → ジョブ（付箋1枚） → タスク（時間を持つ実作業）の3段**。2026-09-15に「タスク」
// 「サブタスク」の呼び名を1段ずつ繰り下げた（旧タスク→ジョブ、旧サブタスク→タスク）。
...
//
// 加えて、板（カテゴリ→ジョブ→タスク）とは完全に独立した**「サブタスク」＝名前だけのTODOリスト**を持つ
// （下記 `KoubaSubtask`）。2026-09-15にタスクへの紐付け・時間集計・DONE機能つきで一度作ったが、
// 2026-09-17に「タスクとの紐付けは不要」という指示で作り直した＝以後、板のどの階層とも一切関わらない。

/**
 * サブタスク（板・タスクとは無関係な、名前だけの独立したTODOリスト。2026-09-17に作り直し）。
 * ドラッグ&ドロップで並べ替えられる（掴めるのは行のチェックボックス・編集アイコン・削除ボタン以外の場所すべて。
 * カテゴリの並べ替えと同じ「掴んだ場所の手前に入る」規則）。
 * PCでは画面左の常設サイドバー、スマホではタブ切り替えで表示する（配置は pages/kouba/index.vue 側）。
 * DBはテーブル `kouba_task_subtasks`（旧・紐付けありサブタスク時代のテーブルをそのまま作り直して流用）。
 */
export interface KoubaSubtask {
  id: string
  title: string
  createdAt: string
  /** 完了（2026-09-18追加）。チェックを入れると一覧下部の「完了済み」（エクスパンドで表示）へ移動する。 */
  done: boolean
}

/**
 * タスク（ジョブの中の実作業。「何をやったか」はタイトルで表す）。時間は日別に分けず hours にまとめて1個持つ
 * （手入力の+/-で自由に増減できる）。サブタスク（`KoubaSubtask`）とは無関係＝板の入れ子には含まれず、別に取得する。
 */
export interface KoubaTask {
  ...
  createdAt: string
  /**
   * 完了（2026-09-20追加）。サブタスクと同じく、チェックを入れるとジョブ詳細モーダルの「完了済み」（折りたたみ）へ移り、
   * 外すと元の位置に戻る。**完了にしても hours はジョブ・カテゴリの合計に残る**（かけた時間は無かったことにならない）。
   */
  done: boolean
}
...
export interface KoubaJob {
  ...
  description: string
  /**
   * 稼働停止中（2026-09-20追加）。ジョブは「終わった」というより「いずれまたやる」ことが多いので、
   * 完了ではなくこの状態にして、板ではカテゴリ枠の下の「稼働停止中」（折りたたみ）へ畳む。
   * 再開すると元の位置に戻る。**停止中でも totalHours は合計に残る**。停止にすると `focused` は外れる。
   */
  paused: boolean
}
