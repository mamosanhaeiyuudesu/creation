...
- Relation: { from: id, to: id, distance: "enmeshed"|"close"|"distant"|"cutoff"(密着度・距離。必須でこの4つから必ず1つ選ぶ。enmeshed=過度に密着・巻き込まれている, close=適度な距離(良好・普通、または対立以外に距離感の手掛かりが無い場合のデフォルト), distant=距離がある・疎遠, cutoff=音信不通・絶縁), conflict?: boolean(対立・不仲・ぶつかるを伴うか。distanceとは独立した軸なので、対立があっても距離が近いか遠いかは別途判断する), dependent?: "from"|"to"|"mutual"(一方または相互の依存を伴うか。"from"=fromがtoに依存, "to"=toがfromに依存, "mutual"=互いに依存し合って抜け出せない(旧"共依存")。依存関係が語られていなければ省略する), label?: string(関係を表す短い日本語、例:"疎遠","対立","絶縁","べったり","共依存") }
...
4. 「仲が悪い」「絶縁」「べったり」「疎遠」「ぶつかる」「共依存」のような感情表現は、relations の distance/conflict/dependent の3つに分けて判断し、label に短い日本語を入れる。
   - distance の目安: べったり・密着・過干渉・一方が相手を巻き込む=enmeshed / 疎遠・距離がある=distant / 絶縁・音信不通=cutoff / それ以外(仲が良い・普通、または対立だけが語られ距離感の手掛かりが無い)=close。
   - conflict の目安: 対立・不仲・ぶつかる・喧嘩する、が語られていれば true(distance の値によらず独立に付けてよい。例:「疎遠なのに顔を合わせると喧嘩になる」は distance: "distant", conflict: true にする)。
   - dependent の目安: 一方だけが相手に依存・世話を焼きすぎている等、依存の向きが片方だけ明確ならその人物を from/to で示す。「共依存」「お互いに依存し合って離れられない・共倒れ」のように双方向なら mutual にする(distance は enmeshed にするのが基本)。依存関係が語られていなければ省略する。
